...
- [x] Admin parity pass (2026-09-19, six branches merged): restored what
      the W6 port had silently dropped from the old admin_app.py —
      **Users**: search/tier filter, delete user & data (guarded against
      user 0 and self), beta-capacity editor, pending login-request
      approve/dismiss, allowlist bulk import (textarea; replaces
      allowed_users.txt reseed). **Stats / Audit / Tiers** pages (fixes
      `changed_from` never being written to SUBSCRIPTION_HISTORY).
      **Demo Content** — is_public toggles scoped to ADMIN-owned rows.
      **System** — migrations pending/preview/history + schema check;
      selective leaderboard evaluation with live progress. **Analytics**
      — ANALYTICS_EVENTS pipeline re-enabled (simulation_run on queue,
      user_login via Better Auth session hook) + dashboard.
      Deliberately NOT ported (obsolete or never live in the old app):
      Git config check, Reseed Strategy Repo, config.yml editor,
      Retention Cleanup stub. See progress log.
...
| 2026-09-19 | W5 | Strategy Q&A built on branch `feature/strategy-qa` (worktree; NOT merged — awaiting Oscar's go). Ask questions about a strategy's observed behavior with prescriptive answers, in the designer (create + evolve, at the review step and after save) and on completed simulation reports. Engine: strategies may return numeric `state_*` keys from `execute_strategy_for_year`; `Portfolio.record_yearly_snapshot` carries them into the yearly history (strings/non-finite dropped so aggregation stays numeric). Designer: the blueprint declares `state_metrics`, generate prompts require them, the test flight reworks code that never records a declared metric, and test-flight artifacts + worst-path trace carry them (evolve runs only extend the set when a planned edit touches `execute_strategy_for_year`, so the minimal-edit guard holds). Q&A: `api/app/qa/` — `QAContext` built from either a generation run (latest code/blueprint/test_flight/behavior artifacts off STRATEGY_GENERATION_EVENTS — nothing new persisted, no sandbox re-run) or a simulation (params, scalar stats, prior Gemini narrative, worst+median sampled paths incl. state rows, built-in class source via inspect); triage (fast, JSON on_topic) → answer (strong, JSON answer_md + optional suggested_change{summary, refine_feedback}); V42 QA_THREADS/QA_MESSAGES (one thread per user per subject); `/qa/thread` + `/qa/messages` behind the internal secret; BFF `/api/bff/qa/*`. UI: `web/components/qa-panel.tsx` reused by `designer.tsx` (refine feedback state lifted to BuildLog so "Use as refine feedback" fills the review textarea) and `simulations/[hash]/page.tsx` (signed-in viewers, no refine button). Gates: api suite green incl. 20 new tests (engine passthrough, rework-on-missing-metric, db round-trip, service triage/answer/credits/ownership, API auth+validation), web lint+build clean, browser-verified on worktree servers 8010/3110 with the canned provider: off-topic refusal, on-topic answer with suggestion → textarea → refine round → reload rehydrates thread → save → evolve run gets its own thread → report page panel; mobile width checked. /code-review high → 5 findings, all fixed on the branch: Q&A turns were persisted before the LLM call (provider outage left a dangling question + generic 500; now both turns land only with an answer, LLMError → 502), state-metric rework was unsatisfiable on parameter-only evolves (the minimal-edit guard forbids touching execute_strategy_for_year; enforcement now skipped where a rework can't add the keys), refine button gated on a stale server can_refine snapshot (now driven by the designer's live review state), <div> inside <ol>, hook-looking handler name. Final gates: 317 api tests green (concurrent pytest sessions on the shared local DB purge each other's @example.com users — run gates alone), lint+build clean, UI re-verified. Not yet exercised against real Gemini (prompt quality unverified — first real question is the next check). |
| 2026-09-19 | W6 | LLM cost tracking on branch `feature/llm-cost-tracking` (worktree), reviewed (/code-review → 11 findings, all fixed on-branch: unscoped drill-down 404, in-flight runs skewing the window, UTC month buckets, N+1 steps fetches, bounded rollup scans, model defaults centralised in `core.llm.model_for_tier`, dead analytics path removed) — awaiting Oscar's merge go-ahead: every Gemini call is written to `LLM_USAGE` from `core.llm.call_gemini_safe` (prompt/cached/completion/*thinking* tokens — thinking was previously discarded and dominates the bill on Flash — cost at the day's list price via `core/llm_pricing.py`, latency, ok/error), attributed to a user-facing operation through `core.llm.llm_scope` (runner: `strategy_create` / `strategy_evolve` keyed by run id, node name as `step`; worker: `report_analysis` keyed by sim hash; Q&A branch to wrap its service call as `strategy_qa` on merge — until then its calls land as `unknown`). `/admin/llm-usage` (+ `/users/[id]`, `/ops/[op]/[ref]`): median/mean/p90/max per operation over the latest N runs, per-step breakdown, spend by day, per-user table, per-call drill-down. Spend rows survive user deletion (SET NULL) so totals stay truthful. `LLM_COST_MODEL.md`: measured prompt sizes → est. $0.085/creation, $0.05/evolution, $0.02/question, $0.01/report on the default models (×3 with the local `.env`'s `gemini-3.1-pro-preview` strong override — production model TBC); 3× markup as credits (1 credit = $0.01 cost; PRO 300/mo, fixed price per operation from p75), free tier 40 credits once + 10/mo; today's 10/50-generation gates lose money from ~1k free users. Gates: 12 new tests (308 green), web lint+build clean, browser-verified on 8020/3120 with seeded rows. |
| 2026-09-19 | WV | Results report clipping fixed (branch `fix/report-accordion-clipping`, MERGED to main per Oscar 2026-09-19). Reported as "plots missing in the UI that the PDF has" (Portfolio Value Overview, Cash Flow & Liabilities, Median Yearly Cash Flow) + "sections get cut": the API item stream already had all 14 charts; `AccordionContent`'s inner div carried a fixed `h-(--radix-accordion-content-height)`, measured once at open while the Plotly charts were still 256px skeletons, so the outer `overflow-hidden` clipped ~1.2k px off Simulation Summary (the last two charts) and 26px off Executive Summary. Dropped the fixed height (stock shadcn; keyframes still animate via the var). Browser-verified: 0px clipped in all chapters, close→reopen, nested Appendices. /code-review low: no findings. |
| 2026-09-19 | W6 | Admin parity pass MERGED to main per Oscar (six single-theme branches, each `/code-review high` → 16 findings total, all fixed on-branch with regression tests; 335 api tests green, web lint+build clean, every page browser-verified as an ADMIN user). Restores what the W6 port left behind from `admin_app.py`: Users & Access extras (search/filter, delete user & data, beta capacity, login-request queue, bulk allowlist import), Stats/Audit/Tiers pages, Demo Content management, migrations inspection view (immediately surfaced `V43__llm_usage.sql` applied-but-not-on-disk drift locally), selective leaderboard evaluation with a reload-safe progress poller, and a Usage Analytics dashboard — which required re-enabling event collection: `AnalyticsService` had no callers since the port and still used SQLite placeholders; `simulation_run` now fires when a job is queued, `user_login` from a Better Auth `session.create.after` hook → `POST /me/login-event` (fire-and-forget, 2s timeout, so the API can never block sign-in). Engine fixes along the way: `update_user_tier` never wrote `changed_from` (audit "From" column always blank, same bug in the old app) and now reports unknown users; `get_all_custom_strategies_for_admin` included soft-deleted rows and the built-ins' user-0 copies (run-all evaluated each built-in twice); `add_allowed_user` returns success/failure instead of swallowing; `delete_users_by_id` raises instead of returning 0, and also clears `analytics_events`. Review findings worth remembering: demo-content toggles were unscoped (could expose a regular user's private strategy), delete had no guard for user 0 / self, a selection filtered through the 60s admin cache silently dropped strategies, `toLocaleString()` in client components hydration-mismatches (use `<time suppressHydrationWarning>`). Skipped on purpose: Git config check + Reseed Strategy Repo (GitHub workflow removed in R4/R5), config.yml editor (was never wired in the old app; wouldn't persist on Cloud Run), Retention Cleanup (old stub always returned 0), second ADMIN_PASSWORD gate on the whole dashboard (tier check stays the only gate; password remains on system reset). |
