...
### W5 — Strategies: agentic generation (biggest single wave)
...
Checklist:
- [ ] Engine prep: seed param on `run_sandbox_test` (paired baseline);
      `_validate_strategy_class` checks `evaluation_category()` (today a
      silent 'HYBRID' fallback); expose per-year traces to the graph;
      prompt API-docs from `inspect.getdoc`.
- [ ] Graph: `api/app/agents/strategy_gen.py` — state, nodes, edges,
      Postgres checkpointer, iteration cap + credit budget.
- [ ] Trace logging from day one: persist every run (spec, retrieved
      examples, each attempt, rung failures) — new table
      `STRATEGY_GENERATION_RUNS`; feeds prompt tuning + grows the golden
      snippet corpus. Cheap now, impossible to backfill.
- [ ] API: strategies CRUD; `POST /strategies/generate` → {run_id};
      `GET /strategies/generate/{run_id}/events` (SSE per-node progress);
      `POST /strategies/generate/{run_id}/resume` (clarify answers /
      review verdict); test-run endpoint; `POST /strategies/evaluate`
      (existing job type) + evaluation results.
- [ ] Web: strategy list; designer per **W5_DESIGNER_UX.md** (build-log
      page: stage rail + accumulating artifact cards — spec/blueprint/
      checks/test-flight/behavior — clarify + review interrupts, SSE with
      checkpoint-rehydrated state); evolve = same designer seeded from an
      existing strategy (code shown as diff); evaluation charts.
- [ ] Tiered models per node (cheap: spec/clarify/analyze; strong:
      generate/rework) — wire the config even if v1 uses one model.
