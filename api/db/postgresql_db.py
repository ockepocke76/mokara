...
class PostgreSQLDatabase(DatabaseInterface):
    ...
    def complete_job(self, job_id, result):
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE id = %s
            """, (json.dumps(_sanitize_for_json(result)), job_id))
            conn.commit()
    ...
    @log_db_call
    def retry_job(self, job_id, error_message, scheduled_at):
        """
        ...
        try:
            ...
            cursor.execute("""
                ...
                SET 
                    ...
                    error_message = %s,
                    scheduled_at = to_timestamp(%s),
                    started_at = NULL,
                    worker_id = NULL
                WHERE id = %s
            """, (error_message, scheduled_at, job_id))
            conn.commit()
    ...
    @log_db_call
    def reset_stale_jobs(self, timeout_seconds=900):
        """
        Reset jobs that have been processing for too long (likely crashed workers).
        
        This is the recovery mechanism for jobs stuck in PROCESSING state.
        Should be run periodically by a cleanup task.
        
        Args:
            timeout_seconds: How long a job can be processing before being reset (default: 15 min)
        ...
        try:
            ...
            cursor.execute("""
                ...
                SET 
                    ...
                    started_at = NULL,
                    worker_id = NULL,
                    error_message = 'Recovered from timeout/crash'
                WHERE status = 'PROCESSING'
                  AND started_at < CURRENT_TIMESTAMP - INTERVAL '%s seconds'
                RETURNING id
            """ % timeout_seconds)  # Note: Using % for INTERVAL, not SQL injection risk
    ...
    @log_db_call
    def update_job_progress(self, job_id, progress_value, progress_message=None):
        """
            ...
            if progress_message:
                cursor.execute("""
                    ...
                    SET payload = jsonb_set(
                        jsonb_set(payload, '{progress_value}', %s::jsonb),
                        '{progress_message}', %s::jsonb
                    ),
                    updated_at = CURRENT_TIMESTAMP
                    WHERE id = %s
                """, (json.dumps(progress_value), json.dumps(progress_message), job_id))
            else:
                cursor.execute("""
                    UPDATE BACKGROUND_JOBS
                    SET payload = jsonb_set(payload, '{progress_value}', %s::jsonb),
                        updated_at = CURRENT_TIMESTAMP
                    WHERE id = %s
                """, (json.dumps(progress_value), job_id))
