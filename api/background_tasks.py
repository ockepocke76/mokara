"""
Long-running engine tasks executed by the background worker:

- run_and_save_simulation — the full simulate→analyze→persist pipeline
- regenerate_ui_results — rebuilds the complete report item stream for a
  stored simulation (also serves GET /simulations/{hash}/report)
- _generate_pdf_for_simulation — renders the PDF report

(The Streamlit-era multiprocessing entry points that used to live here —
queue-fed process targets, the PDF worker loop — were deleted in the R4
cleanup; the job queue in services/background_worker.py is the only
execution path.)
"""
import pandas as pd
import time
